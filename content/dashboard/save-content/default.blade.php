                        ...
                        <div class="col-md-10 offset-md-1">
                            <form action="{{route($saveRoute)}}" method="post" enctype="multipart/form-data">
                                ...
                                <div id="data_fields_create">
                                        ...
                                        <div class="form-group" style="margin-bottom: 60px;">
                                            ...
                                            @else
                                                ...
                                                @elseif ($field->type == 'location')
                                                    ...
                                                    {{isset($content->data_fields[$field->id_name])
                                                      ?(
                                                            array_key_exists('value', $content->data_fields[$field->id_name])
                                                            ?$content->data_fields[$field->id_name]['value']
                                                            :''  
                                                        )
                                                      :''}}"
                                                    ...
                                                    <input type="hidden" id="lat" value="{{ isset($content->data_fields[$field->id_name]) ? $content->data_fields[$field->id_name]['lat'] : '' }}" name="data_fields[{{ $field->id_name }}][lat]" {{ $field->required ? 'required="required"' : '' }}/>
                                                    <input type="hidden" id="lng" value="{{ isset($content->data_fields[$field->id_name]) ? $content->data_fields[$field->id_name]['lng'] : '' }} " name="data_fields[{{ $field->id_name }}][lng]" {{ $field->required ? 'required="required"' : '' }} />
                                                    <input type="hidden" id="streetAddress" value="{{ isset($content->data_fields[$field->id_name]['streetAddress']) ? $content->data_fields[$field->id_name]['streetAddress'] : '' }}"  name="data_fields[{{ $field->id_name }}][streetAddress]" {{ $field->required ? 'required="required"' : '' }} />
                                                    <input type="hidden" id="addressLocality" value="{{ isset($content->data_fields[$field->id_name]['addressLocality']) ? $content->data_fields[$field->id_name]['addressLocality'] : '' }}" name="data_fields[{{ $field->id_name }}][addressLocality]" {{ $field->required ? 'required="required"' : '' }} />
                                                    <input type="hidden" id="addressRegion" value="{{ isset($content->data_fields[$field->id_name]['addressRegion']) ? $content->data_fields[$field->id_name]['addressRegion'] : '' }}" name="data_fields[{{ $field->id_name }}][addressRegion]" {{ $field->required ? 'required="required"' : '' }} />
                                                    <input type="hidden" id="postalCode" value="{{ isset($content->data_fields[$field->id_name]['postalCode']) ? $content->data_fields[$field->id_name]['postalCode'] : '' }}" name="data_fields[{{ $field->id_name }}][postalCode]" {{ $field->required ? 'required="required"' : '' }} />
                                                    <input type="hidden" id="addressCountry" value="{{ isset($content->data_fields[$field->id_name]['addressCountry']) ? $content->data_fields[$field->id_name]['addressCountry'] : '' }}" name="data_fields[{{ $field->id_name }}][addressCountry]" {{ $field->required ? 'required="required"' : '' }} />
                                                @elseif($field->type == 'select')
                                                    ...
                                                    @else
                                                        <select multiple class="demo-default multiple-plugin" placeholder="{{ $field->name }}" id="field_{{ $field->id_name }}" name="data_fields[{{ $field->id_name }}][]" {{ $field->required ? 'required="required"' : '' }}>
                                                            <option value="">Select...</option>
                                                            @foreach (explode(',', $field->attr['tags']) as $tag)
                                                                <option value="{{ $tag }}" {{ isset($content) && in_array($tag, $content->data_fields[$field->id_name]) ? 'selected' : '' }}>{{ $tag }}</option>
                                                            @endforeach
                                                        </select>
                                                    @endif
                                    ...
                                    @endforeach
                                    @if (isset($content) && $content->type_id == 2)
                                        <div class="form-group"><label for="workplace">Arbeitsort</label><input type="text" class="form-control" id="workplace" value="{{ isset($content->data_fields['workplace']) ? $content->data_fields['workplace'] : '' }}" placeholder="Arbeitsort" name="data_fields[workplace]"></div>
                                        <div class="form-group"><label for="contact_info">Kontaktdaten</label><textarea class="form-control" id="contact_info" name="data_fields[contact_info]" rows="3" placeholder="Kontaktdaten">{!! isset($content->data_fields['contact_info']) ? $content->data_fields['contact_info'] : '' !!}</textarea></div>
                                    @endif
                                </div>
                                <br>
                                <br>
