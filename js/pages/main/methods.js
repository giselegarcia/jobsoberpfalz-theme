...
function autocomplete(input, array) {
    ...
    input.addEventListener("input", function(e) {
        ...
        for (let i = 0; i < array.length; i++) {
            let keywords = array[i]["search_terms"] != null?array[i]["search_terms"].replace(/\s/g, '').toUpperCase():"";
            let keywordsTogether = keywords.replace(/,/g, '');
            let wordMaxSize = 0;
            if(keywords.length > array[i]["name"].length) wordMaxSize = keywordsTogether.length;
            else wordMaxSize = array[i]["name"].length;
            for(let j = 0; j < wordMaxSize; j++){
                if (array[i]["name"].substr(j, inputValue.length).toUpperCase() == inputValue) {
                    listItem = document.createElement("a");
                    listItem.setAttribute("class", "result-item");
                    listItem.setAttribute("href", $("meta[name='url-base']").attr('content')+"/arbeitgeber/"+array[i]["slug"]);
                    listItem.innerHTML = array[i]["name"].toLowerCase().replace(inputValue.toLowerCase(), "<strong>"+inputValue.toLowerCase()+"</strong>");
                ...
                if(keywordsTogether.substr(j, inputValue.length).toUpperCase() == inputValue){
                    ...
                    listItem.setAttribute("class", "result-item");
                    listItem.setAttribute("href", $("meta[name='url-base']").attr('content')+"/arbeitgeber/"+array[i]["slug"]);
                    listItem.innerHTML = capitalizeText(array[i]["name"])+" <span>"+capitalizeText(keyword)+"</span>";
                    list.appendChild(listItem);
                    break;
                }
            }
...
function getRequestResponse(method, url, data = {}, token = null){
    if(token != null) return $.ajax({
        url: url,
        type: method,
